...
USAGE: ./diploidify.sh -v|--vcf <path_to_vcf> [--separate-homologs] [-c|--chrom-sizes <path_to_chrom_sizes_file>] [-r|--resolutions resolutions_string] [-p|--psf <path_to_psf>] [--reads-to-homologs <path_to_reads_to_homologs_file>] [--juicer-dir <path_to_juicer_dir>] [--phaser-dir <path_to_phaser_dir>] [-t|--threads thread_count] [-T|--threads-hic hic_thread_count] [--from-stage <stage>] [--to-stage <stage>] <path_to_merged_dedup_bam_1> ... <path_to_merged_dedup_bam_N>
...
if [ -z $chrom_sizes]; then
    chrom_sizes=`parallel --will-cite "samtools view -H {}" ::: $bam | grep "^@SQ" | sort -u | awk -F '\t' -v chr=$chr 'BEGIN{n=split(chr,tmp,"|"); for(i in tmp){chrom["SN:"tmp[i]]=1}}($2 in chrom){len[$2]=substr($3,4)}END{for(i=1;i<=n;i++){print tmp[i]"\t"len["SN:"tmp[i]]}}'`
fi
